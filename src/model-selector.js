...
export class ModelSelector extends LitElement {
    static properties = {
        ...
        modelsError: { type: String },
        ollamaUrl: { type: String },
        showUrlEditor: { type: Boolean },
        tempUrl: { type: String },
        connectionStatus: { type: String },
        newModelMode: { type: Boolean },
        newModelUrl: { type: String },
        newModelProgress: { type: Object },
        newModelError: { type: String },
        onToggleModelSelector: { type: Function },
        onSelectModel: { type: Function },
        onSaveUrl: { type: Function },
        onSaveNewModel: { type: Function },
    ...
    handleToggleModelSelector() {
        if (this.onToggleModelSelector) {
            this.onToggleModelSelector();
        }
    }
    ...
    handleSaveUrl() {
        if (this.onSaveUrl) {
            this.onSaveUrl(this.tempUrl);
        }
    }

    handleSaveNewModel() {
        if (!this.newModelUrl.trim() || this.newModelProgress) return;
        if (this.onSaveNewModel) {
            this.onSaveNewModel(this.newModelUrl);
        }
    }

    handleLoadModels() {
        if (this.onLoadModels) {
            this.onLoadModels();
        }
    }
    ...
    render() {
        return html`
            <div class="model-selector">
                <button class="model-trigger" @click=${this.handleToggleModelSelector} ?disabled=${this.modelsLoading}>
                    ...
                    <div class="model-dropdown">
                        <div class="server-config-compact">
                            ${this.showUrlEditor ? html`
                                ...
                                <input
                                    type="text"
                                    .value=${this.tempUrl}
                                    @input=${(e) => this.tempUrl = e.target.value}
                                    placeholder="http://localhost:11434"
                                    class="server-url-input"
                                />
                                <button class="server-action-btn save" @click=${this.handleSaveUrl} title="Save">✓</button>
                                <button 
                                    class="server-action-btn cancel" 
                                    @click=${() => this.showUrlEditor = false} 
                                    title="Cancel">✕</button>
                            ` : html`
                        ...
                        ` : this.models.length > 0 ? html`
                            <div class="models-grid scrollable-model-list">
                                ${this.filteredModels.map(model => html`
                                    <button
                                        class="model-card single-line ${model.id === this.selectedModel ? 'selected' : ''}"
                                        @click=${() => this.handleSelectModel(model.id)}
                                    >
                            ...
                            if (e.key === "Escape") {
                                ...
                                this.newModelError = null;
                                        ...
                                        <button
                                            ...
                                            @click=${this.handleSaveNewModel}
                                            ?disabled=${!this.newModelUrl.trim() || !!this.newModelProgress}
                            ...
                            this.newModelError = null;
                                            ...
                                            title="Cancel">✕</button>
                                        ${this.newModelProgress ? html`
                                            <span class="new-model-progress">
                                                ${this.newModelProgress.status}
                                                ${typeof this.newModelProgress.percent === "number" ?
                                ` (${this.newModelProgress.percent}%)` : ''}
                                            </span>
                                        ` : ''}
                                        ${this.newModelError ? html`
                                            <span class="new-model-error">${this.newModelError}</span>
                                        ` : ''}
                                    </div>
                                ` : html`
                                    <button
                        ...
                        ` : html`
                            <div class="no-connection">
                                ...
                                <button @click=${this.handleLoadModels} class="retry-btn-compact">↻ Retry</button>
                            </div>
                        `}
                    </div>
                ` : ''}
